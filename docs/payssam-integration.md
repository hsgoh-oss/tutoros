...
- 현금영수증(`receipt`)·잔액(`point`)은 검수 5종에는 없지만 B-07(증빙)·운영 준비 확인용으로
  같은 스크립트에서 실행할 수 있다: `pnpm payssam:smoke receipt <BILL-ID> --trader 0 --number <휴대폰> --tax-type exempt`
...
- 현금영수증 `issue`의 `supplyPrice`·`tax`는 **반드시 명시한다**. 2026-09-15 샌드박스에서
  생략 시 `VALIDATION_002` / `supplyPrice는 필수값입니다.`를 확인했다. 면세는 공급가액=금액,
  세액=0이며, 과세는 부가세 포함 금액을 공급가액과 세액으로 분리한다. 관리자가 발급 전
  사업장의 면·과세 구분을 직접 선택한다.
...
## 8. 계좌이체 현금영수증 (2026-09-15)

공식 규격: [V2 현금영수증 API](https://developers.payssam.kr/api/api-v2/cash-receipt).

### 사용 경로

관리자 **청구·결제 → 계좌이체 청구 상세 → 완납 처리 → 현금영수증**에서 개인 소득공제 또는
사업자 지출증빙을 선택하고 식별번호와 면세·과세 구분을 입력한다. 요청 내용을 확인한 뒤 발급한다.
발급 승인번호·구분·시각은 관리자와 납부자 포털에 표시된다. 취소는 수납 환불과 별도이며,
환불된 청구에서도 기존 영수증의 대조·취소가 가능하다. 카드 결제 청구는 신규 현금영수증 발급 대상에서 제외한다.

### 거래 식별과 결과 확인

- `/bill`을 먼저 발송하지 않아도 고유 `billId`로 현금영수증을 단독 발급할 수 있음을
  샌드박스의 1,000원 발급 → 조회(F) → 취소 → 조회(C)로 확인했다. 테스트 영수증은 취소했다.
- `00030_cash_receipt_operations.sql`을 적용한다. 영수증 전용 `cash_receipt_bill_id`를 외부 호출
  **전에** 저장하며, 결제용 `bill_id`와 수납 상태는 바꾸지 않는다. 기존 발급 건의 조회·취소는 기존 ID를 이어 쓴다.
  취소 후 재발급에 같은 ID를 쓰면 `BILL_001`로 거절되므로, 실제 취소 이력을 확인한 다음 새 ID로 발급한다.
  이전 ID·승인번호는 감사 이력에 보존한다.
- `cash_receipt_operation_id` 조건부 갱신으로 동일 청구의 발급·취소·대조를 직렬화한다.
  통신 오류·승인 정보 누락·발급 후 DB 저장 실패는 처리권을 유지하고 확인 업무를 남긴다.
- 처리 중에는 재발급·재취소를 막는다. 1분 후 **결제선생과 대조**로 실제 승인 이력을 확인한다.
  이전 요청의 결과를 확인하지 못하면 처리권을 풀지 않는다. 운영자는 결제선생에서 이력을 확인해야 한다.
- 대조 시 거래 ID·금액·상태·승인번호·발급 구분·일시를 검증한다. 누락된 `info`를 미발급으로
  간주하지 않으며, 같은 시각의 발급·취소처럼 순서를 정할 수 없는 이력은 확인 대상으로 남긴다.
  상태가 같아도 승인번호와 발급 시각은 갱신한다. 샌드박스는 이력 항목의 `billId`를 생략하고
  `apprDt`를 `YYMMDDhhmmss`로 반환하므로 상위 `billId`를 대조하고 12자리/14자리 날짜를 모두 처리한다.
- 발급번호는 요청에만 사용하고 DB/감사 로그에 저장하지 않는다. API 요청·응답 원문도 로그에 남기지 않는다.
- 사업장은 인증된 세션 또는 콜백 결제 행의 테넌트로 정한다. 발송·조회·환불·영수증에 같은 계정 해석을 사용한다.

### 검증과 운영 전환

`pnpm verify:cash-receipts`는 실제 서버 액션과 API 클라이언트를 가짜 DB·제공자와 실행한다.
중복 발급, 다른 테넌트 접근, 감사/저장 실패, 결과 불명, 잘못된 이력, 승인정보 갱신, 환불 후 취소를 검증하며
외부 API를 호출하지 않는다. 로컬 관리자 화면에서도 샌드박스 발급·조회·취소를 검증한다.

현재 프로젝트의 연동 환경은 샌드박스다. 운영 DB에 00030 마이그레이션을 적용하고, 결제선생에서
발급받은 운영 API 키·URL 및 사업장 매핑으로 전환해야 실제 발급을 사용할 수 있다(§6).
