...
## 개통 막힘 — POINT_001 (2026-08-26 실측·개발자센터 명세 대조)

운영 API Key를 받아 연결한 뒤, 청구서 발송이 아래로 거절된다.

```
POST /bill  →  {"code":"POINT_001","message":"must not be null","data":null}
```

### 명세 대조 — 요청은 정확하다

developers.payssam.kr `/api/api-v2/send.md` 기준 필수 11종을 **전부** 보내고 있다:
`apiKey` `member` `merchant` `billId` `sendType` `productName` `price` `memberName`
`phone` `hash` `callbackUrl`. hash 규칙(`{billId},{phone},{price}` SHA-256)도 일치한다.
`billIssuer` `supplyPrice` `tax` `message` `expireDt` `pageRedirectUrl`은 옵션이라 생략해도 된다.

**요청 형식 문제가 아니다.**

### POINT_001의 뜻과 message 불일치

`/api/api-v2/error-code.md`에 **POINT_001 = "포인트가 부족합니다"**로 명시돼 있다.
그런데 실제 응답 message는 `must not be null`이다 — 문서화된 문구가 아니고, Bean Validation의
기본 문구다. **코드와 문구가 어긋난다**는 사실 자체가 단서다: 잔액을 비교한 결과가 아니라,
**차감할 포인트 계정을 찾다가 null을 만난 것**으로 읽힌다.

### 잔액은 양쪽 다 있다

잔액 조회 엔드포인트는 둘이고, 보는 계정이 다르다(`/api/api-v2/ssam-point.md`):

| 엔드포인트 | 보내는 것 | 보는 계정 | 실측 |
|---|---|---|---|
| `/read/remain_count` | `apiKey` | 파트너 관리 사업장 | 550,000 P |
| `/read/merchant/remain_count` | `apiKey`+`member`+`merchant` | 하위사업장 | 4,885 P |

둘 다 55 P짜리 발송을 감당한다. **단순 잔액 부족이 아니다.**

### 남는 결론

가이드가 이렇게 적고 있다:

> API 호출 시 차감되는 사업장은 **파트너제휴 계약 시 설정한 방식**에 따라 결정됩니다.
> (파트너 일관 관리 / 사업장 개별 관리)

이 설정이 비어 있으면 차감 대상 조회가 null이 되고, 그 결과가 POINT_001 + `must not be null`로
나타난다 — 관측된 모든 사실과 맞아떨어진다. **코드로 고칠 수 없고, 페이민트가 계약 설정을
채워야 한다.**

문의 문안:

> 하위사업장(member·merchant = `axiommathlab`)으로 `POST /bill`을 호출하면
> `{"code":"POINT_001","message":"must not be null"}`으로 거절됩니다.
> - 필수 11종을 명세대로 모두 전송하고 있고 hash 규칙도 일치합니다.
> - `/read/remain_count` = 550,000 P, `/read/merchant/remain_count` = 4,885 P — **양쪽 다 잔액이 있습니다.**
> - `/bill/read`는 `BILL_003`까지 정상 도달합니다(자격증명·사업장 정상).
> - `sendType`이 TALK든 URL이든 동일하게 거절됩니다.
>
> 문서상 POINT_001은 "포인트가 부족합니다"인데 실제 message는 `must not be null`입니다.
> **파트너제휴 계약의 쌤포인트 차감 방식(파트너 일관 관리 / 사업장 개별 관리)이 설정되어 있는지**
> 확인 부탁드립니다.

### 앞선 오판 기록 (2026-08-26 오전)

- ❌ "URL 발송은 포인트를 쓰지 않으므로, URL도 실패하면 포인트 문제가 아니다"
  → **틀렸다.** 가이드상 TALK·URL 모두 55 P다. URL 실패는 포인트 가설을 뒷받침하는 근거였다.
- ❌ "화면의 550,000 P를 보고 포인트가 충분하다"
  → 그 값은 `apiKey`만 보내는 **파트너** 잔액이었고, 하위사업장은 따로 조회해야 했다.
  코드에 하위사업장 조회가 아예 없어 화면이 한쪽만 보여주고 있었다(지금은 둘 다 표시).

### 그동안

청구는 계좌이체(무통장입금) 수단으로 처리한다. 결제선생 수단 청구는 만들어 두되 발송하지 않는다.
