...
## 개통 막힘 — POINT_001 (2026-08-26 실측)

운영 API Key를 받아 연결한 뒤, 카카오톡 청구서 발송이 아래로 거절된다.

```
POST /bill  →  {"code":"POINT_001","message":"must not be null","data":null}
```

`must not be null`은 필드명을 알려주지 않아 원인을 좁히려면 **어떤 호출이 되고 어떤 호출이
안 되는지**를 갈라야 한다. 운영에서 실제로 재본 결과:

| 호출 | 요청에 담기는 것 | 결과 |
|---|---|---|
| `/read/remain_count` | apiKey | ✅ 200 · 잔액 550,000 P |
| `/bill/read` (없는 billId) | apiKey + member + merchant | ✅ 사업장 통과 → `BILL_003 청구서를 찾을 수 없습니다` |
| `/bill` `sendType=TALK` | 위 + 청구 내용 | ❌ `POINT_001` |
| `/bill` `sendType=URL` | 위와 동일(포인트 미차감) | ❌ `POINT_001` |

읽는 법:

- `/bill/read`가 **BILL_003**(사업장 안에서 청구서를 못 찾음)까지 갔다는 건 apiKey·baseUrl·
  member·merchant가 **전부 정상 해석됐다**는 뜻이다. 자격증명 문제가 아니다.
- 포인트는 카카오톡 발송에서만 차감되는데(건당 55 P) **URL 방식도 같은 오류**가 난다.
  즉 "잔액이 모자란다"가 아니라 **청구서 발행 시작 시 조회하는 포인트 계정 자체가 없다**.
- `/read/remain_count`는 스펙상 apiKey만 받는다 — 화면에 뜨는 잔액은 **파트너 계정 기준**이고
  하위사업장의 잔액이 아니다. 550,000 P가 보인다고 하위사업장에 포인트가 붙어 있다는 뜻이 아니다.

**결론: 코드로 못 고친다.** 하위사업장에 쌤포인트 계정을 연결·충전해야 한다(페이민트 작업).

문의 문안:

> 하위사업장(회원ID·상점ID = `axiommathlab`)으로 `/bill`을 호출하면 `sendType`이 TALK든 URL이든
> `POINT_001 / must not be null`로 거절됩니다. 같은 자격증명으로 `/read/remain_count`는 550,000 P를
> 정상 응답하고, `/bill/read`도 `BILL_003`까지 정상 도달합니다.
> 이 하위사업장에 쌤포인트 계정이 연결되어 있는지 확인 부탁드립니다.

### 우회로

풀릴 때까지는 **결제 상세의 「링크만 발급」**(`sendType=URL`)이 대안이다 — 포인트를 쓰지 않고
청구서 단축 URL만 받아 운영자가 직접 전달하며, 결제·콜백·수납은 카카오톡 발송과 동일하게 이어진다.
다만 위 표대로 **지금은 이 경로도 같은 이유로 막혀 있다** — 포인트 계정이 붙어야 둘 다 열린다.
그 전까지 청구는 계좌이체(무통장입금) 수단으로 처리한다.
