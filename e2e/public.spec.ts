...
test.describe("공개 사이트", () => {
  test("홈: 히어로 h1 + 상담 CTA 렌더", async ({ page }) => {
    ...
    await expect(
      page.getByRole("link", { name: "상담 신청하기" }).first(),
    ).toBeVisible();
  });

  test("공개 페이지 전수: 각 페이지 h1 렌더 + 200 응답", async ({ page }) => {
    test.setTimeout(150_000); // dev 콜드 컴파일 다중 네비 여유
    const paths = [
      "/tutor",
      "/classes",
      "/case",
  ...
  test("수업료 계산기: 스테퍼 재계산 · 경계값 비활성 · 프리필", async ({ page }) => {
    ...
    await expect(calc.getByRole("link", { name: "이 구성으로 상담 신청" })).toHaveAttribute(
      "href",
      "/apply?mode=video&hours=3&freq=3",
    );
